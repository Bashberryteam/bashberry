...
const testimonials = [
  { quote: "Bash Berry Xpert completely transformed our technical SEO infrastructure. Within 4 months, our organic revenue doubled. Their attention to detail is remarkable.", name: "Glamira", role: "Jewelry & Accessories" },
  ...
  { quote: "Their grasp on the Google and Meta ecosystem is unparalleled. They look at our entire unit economics to scale profitably.", name: "Zevrik", role: "Fashion Jewelry" },
  ...
  { quote: "Professional, data-driven, and results-focused. They delivered a complete brand overhaul that increased our sales by 145%.", name: "Domino Style", role: "Fashion & Apparel" },
  { quote: "They unlocked Google Merchant Center for us when three other agencies couldn't. Absolute game-changer for our business.", name: "Gigi Boutique", role: "Fashion Boutique" },
  { quote: "Our ROAS consistently exceeds 8x thanks to their Meta ads strategy. They truly understand how to scale fashion brands.", name: "Spirit Fashion", role: "Streetwear" },
  { quote: "From branding to SEO to paid ads, they handle everything seamlessly. Having one team manage our entire growth stack is incredible.", name: "Boutique Chic", role: "Luxury Retail" },
  ...
  { quote: "They scaled our Google Shopping campaigns from $2K to $25K/month spend while maintaining a 6x ROAS. Exceptional work.", name: "Culture Kings", role: "Streetwear Culture" },
  { quote: "The team's understanding of streetwear culture and digital marketing is rare. Our brand awareness skyrocketed.", name: "Staple Pigeon", role: "Urban Fashion" },
  { quote: "Pink Lily's growth story wouldn't be the same without Bash Berry. They helped us crack influencer marketing at scale.", name: "Pink Lily", role: "Women's Fashion" },
  { quote: "As a sustainable brand, we needed an agency that understood our values. They delivered growth without compromising our mission.", name: "GOEX Apparel", role: "Sustainable Fashion" },
  { quote: "Our custom jewelry line needed specialized marketing. Bash Berry delivered a strategy that increased our average order value by 85%.", name: "Miaymax Jewelry", role: "Custom Jewelry" },
  { quote: "The level of care they put into our email marketing sequences is impressive. Our email revenue now accounts for 35% of total sales.", name: "Marrow Fine", role: "Fine Jewelry" },
  { quote: "They rebuilt our entire online presence and the results speak for themselves — 280% revenue growth in one year.", name: "Dayyani Jewelers", role: "Luxury Jewelry" },
  { quote: "Bash Berry's SEO work put us on the first page for our most competitive keywords. The organic traffic boost has been phenomenal.", name: "Shane Co.", role: "Diamond Jewelry" },
  ...
  { quote: "Kenneth Rebels grew 195% in revenue within 8 months of partnering with Bash Berry. Their framework simply works.", name: "Kenneth Rebels", role: "Lifestyle Brand" },
];
...
const Home = () => {
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero */}
      <section className="noise-bg relative min-h-screen bg-hero-gradient pt-16">
        <div className="container relative z-10 mx-auto flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center px-6 text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="mb-6 max-w-4xl text-5xl font-extrabold leading-tight tracking-tight text-foreground md:text-7xl">
            The Complete Shopify{" "}<span className="text-gradient">Sales Growth Engine.</span>
          </motion.h1>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="mb-10 max-w-2xl text-lg leading-relaxed text-muted-foreground">
            We don't just build stores; we scale them. From technical design to Google/Meta Ecosystem mastery, we turn browsers into loyal buyers.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mb-8 flex flex-col items-center gap-4 sm:flex-row">
            <Link to="/services" className="bg-gradient-silver flex items-center gap-2 rounded-full px-8 py-4 text-base font-semibold text-primary-foreground transition-all hover:shadow-glow">
              View Our Growth Framework <ArrowRight className="h-5 w-5" />
            </Link>
            <Link to="/portfolio" className="flex items-center gap-2 rounded-full border border-border bg-secondary px-8 py-4 text-base font-semibold text-foreground transition-all hover:border-foreground/30">
              View Portfolio
            </Link>
          </motion.div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-6">
            {badges.map((badge) => (
              <div key={badge} className="flex items-center gap-2 text-sm text-muted-foreground">
                <CheckCircle2 className="h-4 w-4 text-foreground" /> {badge}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative overflow-hidden">
          <div className="animate-marquee flex w-max gap-16 whitespace-nowrap">
            {[...brands, ...brands].map((brand, i) => (
              <a key={i} href={brand.url} target="_blank" rel="noopener noreferrer" className="text-xl font-semibold text-muted-foreground/40 transition-colors hover:text-foreground cursor-pointer">
                {brand.name}
              </a>
            ))}
          </div>
      ...
      <section className="noise-bg bg-section-alt py-24">
        <div className="container relative z-10 mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <p className="mb-2 text-center text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Why Choose Us</p>
...
const TestimonialSlideshow = ({ testimonials }: { testimonials: Testimonial[] }) => {
  ...
  return (
    <section className="bg-background py-24">
      <div className="container mx-auto px-6">
        <p className="mb-2 text-center text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Client Testimonials</p>
        ...
        <div className="mx-auto max-w-2xl text-center">
          <motion.div key={current} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="rounded-xl border border-border bg-card p-10">
            <div className="mb-6 flex justify-center gap-1">
              {[...Array(5)].map((_, i) => <Star key={i} className="h-5 w-5 fill-foreground text-foreground" />)}
            </div>
            <p className="mb-8 text-lg leading-relaxed text-muted-foreground italic">"{t.quote}"</p>
            <div className="flex items-center justify-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-silver text-sm font-bold text-primary-foreground">{t.name[0]}</div>
          ...
          <div className="mt-8 flex justify-center gap-1.5 flex-wrap">
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className={`h-2 rounded-full transition-all ${i === current ? "w-8 bg-foreground" : "w-2 bg-muted"}`} />
            ))}
          </div>
        </div>

        <div className="mt-12 text-center">
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-full border border-border bg-secondary px-6 py-3 text-sm font-medium text-foreground transition-all hover:border-foreground/30">
