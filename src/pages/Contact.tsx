...
const serviceOptions = ["Web Development", "SEO", "Meta Ecosystem Strategy", "Google Ecosystem Strategy", "Email Marketing", "Reputation Management", "The 5-Pillar Growth Framework", "Others"];
const budgetOptions = ["Under $500", "$500 - $1,500", "$1,500 - $3,000", "$3,000 - $5,000", "$5,000 - $10,000", "$10,000+"];

const inputCn = "w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:border-foreground/30 focus:outline-none focus:ring-1 focus:ring-foreground/10 transition-colors";
const selectCn = "w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm text-foreground focus:border-foreground/30 focus:outline-none focus:ring-1 focus:ring-foreground/10 transition-colors appearance-none";

const Contact = () => {
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", storeUrl: "", service: "", budget: "", challenge: "" });
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="noise-bg bg-hero-gradient pt-32 pb-20">
        <div className="container relative z-10 mx-auto px-6 text-center">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Let's Talk</motion.p>
          ...
          <p className="mx-auto max-w-2xl text-lg text-muted-foreground">Partner with an expert team that understands the full e-commerce ecosystem.</p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className="grid gap-12 lg:grid-cols-2">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <p className="mb-8 text-muted-foreground leading-relaxed">Whether you're launching a new store or scaling an existing one, we'll help you identify opportunities and build a comprehensive growth strategy.</p>

              <a href="mailto:bashberryexpert@gmail.com" className="flex items-center gap-4 rounded-xl border border-border bg-card p-4 transition-all hover:border-foreground/30 mb-8">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={step.num} className="flex gap-4">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-silver text-xs font-bold text-primary-foreground">{step.num}</div>
                      <div>
              ...
              <button onClick={() => setShowReview(!showReview)} className="flex items-center gap-2 rounded-full border border-border bg-secondary px-5 py-2.5 text-sm font-medium text-foreground transition-all hover:border-foreground/30">
                <MessageSquare className="h-4 w-4 text-foreground" /> Leave a Review
              </button>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-card p-8">
                ...
                {submitted && (
                  <div className="mb-4 rounded-lg bg-secondary p-3 text-sm text-foreground font-medium">Redirecting to WhatsApp — we'll respond within 24 hours!</div>
                )}
                <div className="space-y-4">
                  ...
                  <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-lg bg-gradient-silver px-6 py-3 text-sm font-semibold text-primary-foreground transition-all hover:shadow-glow">
          ...
          {showReview && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mx-auto mt-16 max-w-lg">
              <form onSubmit={handleReviewSubmit} className="rounded-xl border border-foreground/20 bg-card p-8 shadow-glow">
                <h3 className="mb-6 text-lg font-semibold text-foreground flex items-center gap-2">
                  <Star className="h-5 w-5 text-foreground" /> Leave a Review
                </h3>
                {reviewSubmitted && (
                  <div className="mb-4 rounded-lg bg-secondary p-3 text-sm text-foreground font-medium">Thank you for your review!</div>
                )}
                <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <button key={n} type="button" onClick={() => setReviewForm({ ...reviewForm, rating: n })} className="transition-transform hover:scale-110">
                          <Star className={`h-6 w-6 ${n <= reviewForm.rating ? "fill-foreground text-foreground" : "text-muted"}`} />
                        </button>
                      ))}
                    </div>
                  </div>
